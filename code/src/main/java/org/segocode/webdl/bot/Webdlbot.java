...
import org.segocode.webdl.bot.model.User;
...
public class Webdlbot extends TelegramLongPollingBot {
    ...
    private void initializeRootContainer() {
        ...
        LOGGER.info(
                "Root db object contains {} users",
                ((DataRootContainer) storageManager.root()).getUsers().size());
    }
                ...
                executorService.submit(() -> {
                    try {
                        if (queuedMessageId != null) {
                            execute(MessageService.deleteMessage(
                                    update.getMessage().getChatId(), queuedMessageId));
                        }
